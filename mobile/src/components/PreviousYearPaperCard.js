...
function PreviousYearPaperCard({
  ...
  const title = formatTaxonomyLabel(item?.title || 'Previous year paper');
  ...
  const itemId = String(item?._id ?? title);
  ...
  useDevItemMountCounter('PreviousYearPaperCard', itemId, { logEvery: 16 });
  ...
  return (
    ...
    >
      <View style={styles.topRow}>
        <View style={styles.titleBlock}>
          ...
          <View style={styles.examMetaRow}>
            {year != null ? <Text style={styles.examMeta}>{year}</Text> : null}
            {year != null && qCount > 0 ? <Text style={styles.examMetaDot}>·</Text> : null}
            {qCount > 0 ? (
          ...
          <View style={styles.chipRow}>
            <View style={styles.typeChip}>
              <Text style={styles.typeChipText}>Previous Year Paper</Text>
            </View>
            {isLoading ? (
      ...
      <Pressable
        ...
        accessibilityLabel={isStarting ? 'Starting paper' : safeLabel}
        style={({ pressed }) => [
          styles.cta,
...
export default memo(PreviousYearPaperCard);
